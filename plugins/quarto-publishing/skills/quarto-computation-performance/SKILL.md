...
description: Use when designing or reviewing Quarto workflows where execution cost, render time, caching, freezing, precomputed artifacts, externalized R computation, simulations, model fitting, repeated or parameterized renders, pipeline-like execution, random seeds, or proportional validation are central. Use for deciding whether computation belongs inside .qmd files, R scripts, stored artifacts, or an external pipeline. Do not use for ordinary .qmd authoring, general chunk-option cleanup, project YAML architecture unrelated to execution cost, format styling, report writing/design, deployment/CI, accessibility, or deep render troubleshooting except when the main issue is excessive recomputation or unsafe long renders.
---
...
2. Decide the appropriate execution strategy: ordinary execution for cheap computations, document/project-level `execute: cache`, cell-level `cache: true`, document/project-level `execute: freeze`, precomputed artifacts, external R scripts, or a pipeline tool.
...
- Use document/project-level `execute: cache` or cell-level `cache: true` for reusable execution results, especially in R/knitr documents.
...
Use `quarto-authoring-core` for ordinary chunk options, inline R, figures, tables, citations, cross-references, and `.qmd` structure.
